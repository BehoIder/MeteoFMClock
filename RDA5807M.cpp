...
bool RDA5807M::setVolume(const byte volume)
{
    if (volume >= RDA5807M_VOLUME_MASK) return false;
    else
    {
...
void RDA5807M::seekUp(bool wrap, void (*showFunc)(word currentFrequency))
{
    updateRegister(RDA5807M_REG_CONFIG, (RDA5807M_FLG_SEEKUP | RDA5807M_FLG_SEEK | RDA5807M_FLG_SKMODE),  (RDA5807M_FLG_SEEKUP | RDA5807M_FLG_SEEK | (wrap ? 0x00 : RDA5807M_FLG_SKMODE)));
    while (!(getRegister(RDA5807M_REG_STATUS) & RDA5807M_STATUS_STC))
    {
        if (showFunc != nullptr) showFunc(getFrequency());
        delay(50);
    }
};

void RDA5807M::seekDown(bool wrap, void (*showFunc)(word currentFrequency))
{
    updateRegister(RDA5807M_REG_CONFIG, (RDA5807M_FLG_SEEKUP | RDA5807M_FLG_SEEK | RDA5807M_FLG_SKMODE),  (0x00 | RDA5807M_FLG_SEEK | (wrap ? 0x00 : RDA5807M_FLG_SKMODE)));
    while (!(getRegister(RDA5807M_REG_STATUS) & RDA5807M_STATUS_STC))
    {
        if (showFunc != nullptr) showFunc(getFrequency());
        delay(50);
...
{
    byte band = getRegister(RDA5807M_REG_TUNING) & (RDA5807M_BAND_MASK | RDA5807M_SPACE_MASK);
...
word RDA5807M::getFrequency(void) 
{
    ...
    return pgm_read_word(&RDA5807M_BandLowerLimits[lowByte(spaceandband)]) + (getRegister(RDA5807M_REG_STATUS) & RDA5807M_READCHAN_MASK) *
...
void RDA5807M::reset(void)
{
    setRegister(0x02, makeWord(0b11000000, 0b00000011));
    setRegister(0x03, makeWord(0b00000000, 0b00000000));
    setRegister(0x04, makeWord(0b00001010, 0b00000000));
    setRegister(0x05, makeWord(0b10001000, 0b00001111));
    setRegister(0x06, makeWord(0b00000000, 0b00000000));
    setRegister(0x07, makeWord(0b01000010, 0b00000010));
}
