...
public:
	...
	void	enable(bool switchOn);  /// Terminate all radio functions.
	...
	void	seekUp(void(*seekProgress)(uint16_t currentFrequency) = NULL);   /// Start a seek upwards from the current frequency.
	void	seekDown(void(*seekProgress)(uint16_t currentFrequency) = NULL); /// Start a seek downwards from the current frequency.
